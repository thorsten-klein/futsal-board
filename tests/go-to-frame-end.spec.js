...
test.describe('Go To Frame End (touch)', () => {
    ...
    test('tapping board after go to frame start navigates to last frame on touch devices', async ({ page }) => {
        const { playerId, parentPos, childPos } = await setupAnimation(page);
        ...
        // Tap on board area using touch — should go to last frame (child board position)
        const bb = await page.locator('.board-container').boundingBox();
        await page.touchscreen.tap(bb.x + 400, bb.y + 400);
        await page.waitForTimeout(100);
